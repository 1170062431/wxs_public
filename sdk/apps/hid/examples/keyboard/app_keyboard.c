...
#include "..\cpu\user\user_project.h"
...
//user add
#define	USER_KEY_POWER_OFF			0x1000
#define	USER_KEY_HOME				0x2000
#define	USER_KEY_LOCK				0x4000
//user add end
...
static void hidkey_app_key_deal_test(u8 key_type, u8 key_value)
{
    ...
    if (key_type == KEY_EVENT_CLICK) {
		...
		case 0:
			YK.F_powerOff = 1;
			YK.set_led_onoff_times = 3;
			IO_LED_P_set;
			break;
		case USER_KEY_LOCK:
			user_send_cmd_prepare(USER_CTRL_HID_RETURN,0,NULL);
			break;
		case USER_KEY_HOME:
			user_send_cmd_prepare(USER_CTRL_HID_HOME,0,NULL);
			break;
		}
		//user add end
    }
