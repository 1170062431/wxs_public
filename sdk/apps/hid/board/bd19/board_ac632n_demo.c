...
static void close_gpio(u8 is_softoff)
{
	...
	//user add
	port_protect(port_group, TCFG_IOKEY_VOLUP_ONE_PORT);
	port_protect(port_group, TCFG_IOKEY_VOLDN_ONE_PORT);
...
struct port_wakeup uport1 = {
	...
	.iomap              = TCFG_ADKEY_PORT,                   //唤醒口选择
#else
	.iomap              = TCFG_IOKEY_VOLUP_ONE_PORT,         //唤醒口选择
#endif
    .filter             = PORT_FLT_2ms,
};

struct port_wakeup uport2 = {
	.pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
	.edge               = FALLING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .both_edge          = 0,
	...
	.iomap              = TCFG_ADKEY_PORT,                   //唤醒口选择
#else
	.iomap              = TCFG_IOKEY_VOLDN_ONE_PORT,         //唤醒口选择
#endif
    .filter             = PORT_FLT_2ms,
};

struct port_wakeup uport3 = {
	.pullup_down_enable = ENABLE,                            //配置I/O 内部上下拉是否使能
	.edge               = FALLING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .both_edge          = 0,
