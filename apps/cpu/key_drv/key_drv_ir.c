...
void timer1_isr_fun(void)
{
    ...
    */
        ...
        ir_code.bState = 0;
    } else if ((cap > 20) && (ir_code.boverflow > 53)) { //溢出情况�?�?2M 48M�?        ir_code.bState = 0;
        ir_code.boverflow = 0;
    }
...
/*----------------------------------------------------------------------------*/
/**@brief   ir按键初始�?   @param   void
   @param   void
   @return  void
   @note    void set_ir_clk(void)

   ((cnt - 1)* 分频�?/lsb_clk = 1ms
*/
/*----------------------------------------------------------------------------*/
#define MAX_TIME_CNT 0x07ff //分频准确范围，更具实际情况调�?#define MIN_TIME_CNT 0x0030
...
void ir_key_init(void)
{
    ...
    IRQ_REQUEST(IRQ_TIME1_IDX, timer1_isr_fun) ; //timer1_isr
...
{
    u8 tkey = 0xff;
//	printf("ir_code:wData = %d, bState = %d, wUserCode = %d, boverflow = %d\n",ir_code.wData,ir_code.bState,ir_code.wUserCode,ir_code.boverflow);
    if (ir_code.bState != 32) {
        return tkey;
    }
