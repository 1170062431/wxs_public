...
u8 user_uart_rxLen = 0;
u8 user_uart_rxBuff[16];
...
void putbyte(char a)
{
//#if ((defined __DEBUG)||(defined USER_UART_EN))
#ifdef __DEBUG
#if (DEBUG_UART == UART0)
    JL_UART0->BUF = a;
...
void user_putbyte(char a)
{
    ...
    while ((JL_UART0->CON0 & BIT(15)) == 0);    //TX IDLE
#elif (USER_UART == UART1)
    JL_UART1->BUF = a;
    __asm__ volatile("csync");

    while ((JL_UART1->CON0 & BIT(15)) == 0);    //TX IDLE
#elif (USER_UART == UART2)
    JL_UART2->BUF = a;
    __asm__ volatile("csync");
...
s32 uart_debug_init(u32 baud_rate)
{
    s32 status = 0;
//UART0 CONFIG
#if (DEBUG_UART == UART0)
...
//UART0 CONFIG
#if (USER_UART == UART0)
#if (USER_UART_SEL == UART0_TXPA5_RXPA6)
    JL_IOMAP->CON0 &= ~(BIT(7) | BIT(6));
    JL_PORTA->OUT |= BIT(5) ;
    JL_PORTA->DIR |= BIT(6) ;
    JL_PORTA->DIR &= ~BIT(5) ;

#elif (USER_UART_SEL ==   UART0_TXPB5_RXPA0)
    JL_IOMAP->CON0 &= ~(BIT(7) | BIT(6));
    JL_IOMAP->CON0 |= BIT(6);
    JL_PORTB->OUT |= BIT(5) ;
    JL_PORTA->DIR |= BIT(0) ;
    JL_PORTB->DIR &= ~BIT(5) ;
#elif (USER_UART_SEL == UART0_TXPC2_RXPC3)
    JL_IOMAP->CON0 &= ~(BIT(7) | BIT(6));
    JL_IOMAP->CON0 |= BIT(7);
    JL_PORTC->OUT |= BIT(2) ;
    JL_PORTC->DIR |= BIT(3) ;
    JL_PORTC->DIR &= ~BIT(2) ;
#elif (USER_UART_SEL == UART0_TXPA7_RXPA8)
    JL_IOMAP->CON0 |= BIT(7) | BIT(6);
    JL_PORTA->OUT |= BIT(7) ;
    JL_PORTA->DIR |= BIT(8) ;
    JL_PORTA->DIR &= ~BIT(7) ;
#elif (USER_UART_SEL ==  UART0_OUTPUT_CHAL)
    JL_IOMAP->CON3 &= ~BIT(3);
    JL_IOMAP->CON1 &= ~(BIT(8) | BIT(9) | BIT(10));
    //任意IO口输出,所选则的IO口需要设置为输出, 且开上下拉.  //这里以PA11为例
    JL_PORTA->DIR &= ~BIT(11);
...
//UART1 CONFIG
#if (USER_UART==UART1)
#if (USER_UART_SEL ==  UART1_TXPB0_RXPB1)
    JL_IOMAP->CON1 &= ~(BIT(3) | BIT(2));
    JL_PORTB->OUT |= BIT(0) ;
    JL_PORTB->DIR |= BIT(1) ;
    JL_PORTB->DIR &= ~BIT(0) ;

#elif (USER_UART_SEL == UART1_TXPC0_RXPC1)
    JL_IOMAP->CON1 &= ~(BIT(3) | BIT(2));
    JL_IOMAP->CON1 |= BIT(2);
    JL_PORTC->OUT |= BIT(0) ;
    JL_PORTC->DIR |= BIT(1) ;
...
#elif (USER_UART_SEL ==  UART1_TXPA1_RXPA2)
    JL_IOMAP->CON1 &= ~(BIT(3) | BIT(2));
    JL_IOMAP->CON1 |= BIT(3);
    JL_PORTA->OUT |= BIT(1) ;
    JL_PORTA->DIR |= BIT(2) ;
...
#elif (USER_UART_SEL ==  UART1_USB_TXDP_RXDM)
    JL_IOMAP->CON1 |= BIT(3) | BIT(2);
...
#elif (USER_UART_SEL ==  UART1_OUTPUT_CHAL)
    JL_IOMAP->CON3 &= ~BIT(7);
    //JL_IOMAP->CON3 &= ~(BIT(22) | BIT(21) | BIT(20));
    JL_IOMAP->CON1 &= ~(BIT(8) | BIT(9) | BIT(10) | BIT(11));
    JL_IOMAP->CON1 |= BIT(8);
...
//UART2 CONFIG
#if (USER_UART == UART2)
#if (USER_UART_SEL == UART2_TXPA3_RXPA4)
    JL_IOMAP->CON1 &= ~(BIT(15) | BIT(14));
    JL_PORTA->OUT |= BIT(3) ;
    JL_PORTA->DIR |= BIT(4) ;
    JL_PORTA->DIR &= ~BIT(3) ;

#elif(USER_UART_SEL ==  UART2_TXPA9_RXPA10)
    JL_IOMAP->CON1 &= ~(BIT(15) | BIT(14));
    JL_IOMAP->CON1 |= BIT(14);
    JL_PORTA->OUT |= BIT(9) ;
    JL_PORTA->DIR |= BIT(10) ;
...
#elif (USER_UART_SEL ==  UART2_TXPB2_RXPB3)
    JL_IOMAP->CON1 &= ~(BIT(15) | BIT(14));
    JL_IOMAP->CON1 |= BIT(15);
    JL_PORTB->OUT |= BIT(2) ;
    JL_PORTB->DIR |= BIT(3) ;
...
#elif (USER_UART_SEL ==   UART2_TXPC4_RXPC5)
    JL_IOMAP->CON1 |= BIT(15) | BIT(14);
    JL_PORTC->OUT |= BIT(4) ;
    JL_PORTC->DIR |= BIT(5) ;
    JL_PORTC->DIR &= ~BIT(4) ;

#elif (USER_UART_SEL ==   UART2_OUTPUT_CHAL)
    JL_IOMAP->CON3 &= ~BIT(11);
    JL_IOMAP->CON1 &= ~(BIT(8) | BIT(9) | BIT(10) | BIT(11));
    JL_IOMAP->CON1 |= 0x07 << 8;
    JL_PORTA->DIR &= ~BIT(11);
    ...
    JL_UART2->CON = BIT(14) | BIT(13) | BIT(12) | BIT(10) | BIT(3) | BIT(0);
#endif  //end of UART2 CONFIG
//	register_handle_printf_putchar(putchar);
//	register_handle_printf_putchar(user_putchar);
...
//UART0 CONFIG
    if (updata_uart.control_io == UART0_TXPA5_RXPA6) {
        ...
        log_printf("update_uart io is err!!!!!!!!");
    }
...
#ifdef USER_UART_EN
    user_uart_init(USER_UART_RATE);
#endif

#ifdef __DEBUG
    uart_debug_init(DEBUG_UART_RATE);
#endif
...
void user_common_uart_isr(u8 uto_buf, void *p, u8 isr_flag)
{
    ...
    if (UART_ISR_TYPE_DATA_COME == isr_flag) {
		edr_fcc_cmd_flag = user_uart_isr_callback(uto_buf);
    }
}

u8 F_bleConnectRequest = 0;
u8 bleConnectReceBuff[10] = {0};
u8 bleConnectSendBuff[10] = {0};
u32 deviceConnectValueA,deviceConnectValueB;
u8 user_uart_isr_callback(u8 uto_buf)
{
	/*	test
	u8 temp;
    temp = uto_buf;
	user_uart_write(temp);
//	*/
	/*
	if(F_bleConnectRequest && (uto_buf == 0xAA))
	{
		F_bleConnectRequest = 0;
		F_bleConnectReceStart = 1;
		F_bleConnectReceNum = 0;
	}
	if(F_bleConnectReceStart)
	{
		bleConnectReceBuff[F_bleConnectReceNum] = uto_buf;
		F_bleConnectReceNum++;
		if(F_bleConnectReceNum == 4)
		{
			task_post_msg(NULL, 1, MSG_BLE_CONNECT_RECEIVED);
			F_bleConnectReceStart = 0;
		}
	}
//	*/

//	/*
//	if(!F_uart2_state_rec_start && uto_buf == 0xAB)
#if(PRO_TYPE == PRO_JTJ)
	if(uto_buf == 0xAB)
	{
		F_uart2_state_rec_start = 1;
		uart2_state_rec_num = 0;
	}
	if(F_uart2_state_rec_start)
	{
		user_ble_txBuff[uart2_state_rec_num] = uto_buf;
		uart2_state_rec_num++;
		if(uart2_state_rec_num == (user_ble_txBuff[state_length]+4))
		{
			user_uart_rxLen = uart2_state_rec_num;
		#if (PRO_TYPE == PRO_JTJ)
			if(user_ble_txBuff[user_uart_rxLen-2] != user_ble_txBuff[user_uart_rxLen-1])
				task_post_msg(NULL, 1, MSG_BT_JTJ_CMD_FB);
			else
				task_post_msg(NULL, 1, MSG_BT_JTJ_STATE_UPDATE);
		#endif
		}
	}
#endif
//	*/
	return user_uart_rxLen;
}
